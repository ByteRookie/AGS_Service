class AgsMediaCard extends HTMLElement {
  ...
  renderPlayerSection(ags, control) {
    if (ags.state === "off") {
      return `
        <div class="system-off-view">
          <div class="off-icon-wrap">
            <ha-icon icon="mdi:power-sleep"></ha-icon>
          </div>
          <div class="off-text">AGS System is Off</div>
    ...
    const title = isTv ? "Television Audio" : (control?.attributes?.media_title || "Idle");
      ...
      <div class="player-view">
        ...
        <div class="playback-controls">
          ...
          <div class="buttons-row">
            ...
            <button class="play-btn" onclick="this.getRootNode().host.callService('media_player', 'media_play_pause', {entity_id: '${ags.entity_id}'})">
  ...
  renderVolumesSection(ags) {
    ...
    return `
      <div class="volumes-view">
        <div class="view-title">Volumes</div>
        <div class="list-card master-vol-card">
          <div class="vol-label-row"><span>Master Control</span><span>${groupVol}%</span></div>
            ...
            return `
              <div class="list-card" style="margin-bottom:8px; padding:12px 16px;">
                ...
                <div class="volume-inline">
                  <input class="volume-slider" type="range" min="0" max="100" value="${v}" ${!spkId ? 'disabled' : ''} onchange="this.getRootNode().host.callService('media_player', 'volume_set', {entity_id: '${spkId}', volume_level: this.value/100})" />
                </div>
              </div>
            `;
          }).join("")}
  ...
  renderBrowse() {
    return `<div class="browse-view"><div style="display:flex; align-items:center; gap:8px; margin-bottom:12px;">
      ...
      <div class="browse-list">${this._browseItems.map((i, idx) => `
        <div class="list-card browse-item" onclick="this.getRootNode().host._handleBrowseClick(${idx})">
          ${i.thumbnail?`<img src="${i.thumbnail}"/>` : `<ha-icon icon="${i.can_expand?'mdi:folder':'mdi:music-note'}" style="opacity:0.4;"></ha-icon>`}
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        ha-card { 
          position: relative; overflow: hidden; border-radius: 28px; background: var(--card-bg); color: var(--text); 
          max-width: 400px; margin: 0 auto; aspect-ratio: 0.7 / 1; display: flex; flex-direction: column; 
          border: 1px solid var(--divider); box-shadow: var(--ha-card-box-shadow, 0 4px 20px rgba(0,0,0,0.1));
          transition: background 0.3s, color 0.3s;
        }
        .backdrop { 
          position: absolute; inset: -20px; 
          background-image: ${pic ? `url(${pic})` : 'none'}; 
          background-size: cover; background-position: center; 
          filter: blur(40px) saturate(1.5) brightness(var(--backdrop-brightness, 0.8)); 
          opacity: 0.5; z-index: 0; transition: 0.8s;
        }
        @media (prefers-color-scheme: dark) { 
          :host { --text: #cecece; --text-sec: #9e9e9e; --divider: rgba(255,255,255,0.1); }
          ha-card { --glass: rgba(255,255,255,0.08); --glass-heavy: rgba(255,255,255,0.12); }
          .backdrop { --backdrop-brightness: 0.3; opacity: 0.6; } 
        }
        @media (prefers-color-scheme: light) { 
          .backdrop { --backdrop-brightness: 1.1; opacity: 0.2; } 
        }
        ...
        .header-picker-wrap { display: flex; align-items: center; gap: 10px; flex: 1; min-width: 0; }
        .source-select-mini { 
          width: auto; max-width: 110px; height: 26px; border-radius: 6px; 
          background: var(--glass-heavy); border: 1px solid var(--divider); 
          color: var(--text); padding: 0 4px; font-weight: 900; font-size: 0.7rem; 
          outline: none; appearance: none; text-overflow: ellipsis; cursor: pointer;
        }
        .header-rooms { font-size: 0.8rem; font-weight: 800; color: var(--primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1; }

        .header-actions { display: flex; align-items: center; gap: 4px; flex-shrink: 0; }
        
        .section-body { flex: 1; padding: 12px 20px; overflow-y: auto; scrollbar-width: none; }
        .section-body::-webkit-scrollbar { display: none; }
        
        .list-card { background: var(--glass); backdrop-filter: blur(10px); border: 1px solid var(--divider); border-radius: 16px; }
        ...
        .idle-art { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; color: var(--text); opacity: 0.25; }
        .idle-art ha-icon { --mdc-icon-size: 64px; }
        
        .track-info { text-align: center; margin-bottom: 12px; padding: 0 10px; }
        .track-title { font-size: 1.2rem; font-weight: 900; letter-spacing: -0.02em; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: var(--text); }
        .track-subtitle { font-size: 0.85rem; color: var(--text-sec); font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .play-btn { width: 56px; height: 56px; border-radius: 50%; background: var(--primary); color: #fff; border: none; display: flex; align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 4px 12px rgba(var(--rgb-primary-color, 0,0,0), 0.3); }
        ...
        .fav-art-shell { aspect-ratio: 1; border-radius: 12px; background: var(--glass); border: 1px solid var(--divider); display: flex; align-items: center; justify-content: center; margin-bottom: 4px; }
        .fav-label { font-size: 0.65rem; font-weight: 700; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: var(--text-sec); }
        
        .browse-item { display: flex; align-items: center; gap: 10px; padding: 10px 12px; margin-bottom: 6px; cursor: pointer; transition: 0.2s; border-radius: 12px; }
        ...
        .system-off-view { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; gap: 20px; }
        .off-icon-wrap { width: 100px; height: 100px; border-radius: 50%; background: var(--glass-heavy); display: flex; align-items: center; justify-content: center; color: var(--text-sec); }
        .off-icon-wrap ha-icon { --mdc-icon-size: 48px; opacity: 0.5; }
        .off-text { font-size: 1.2rem; font-weight: 800; color: var(--text-sec); }
        .turn-on-btn { width: auto; height: auto; padding: 12px 32px; border-radius: 16px; display: flex; align-items: center; gap: 12px; font-weight: 800; font-size: 1rem; }
      </style>
      <ha-card>
        ...
        <div class="surface">
          <div class="card-header">
            <div class="header-picker-wrap">
              <select class="source-select-mini" onchange="this.getRootNode().host.callService('media_player', 'select_source', {entity_id: '${ags.entity_id}', source: this.value})">
                <option value="" disabled ${!currentSrc ? 'selected' : ''}>${this.escapeHtml(currentSrc)}</option>
                <optgroup label="Favorites">
                  ${agsSources.map(s => `<option value="${s.name}" ${currentSrc === s.name ? 'selected' : ''}>${this.escapeHtml(s.name)}</option>`).join("")}
                </optgroup>
                <optgroup label="Inputs">
                  ${nativeSources.filter(s => !agsSources.find(as => as.name === s)).map(s => `<option value="${s}" ${currentSrc === s ? 'selected' : ''}>${this.escapeHtml(s)}</option>`).join("")}
                </optgroup>
              </select>
  ...
  renderFavorites(ags) {
    ...
    return `<div class="favorites-view"><div class="view-title">Favorites</div><div class="fav-grid">${s.map(f => `
      <div class="fav-item" onclick="this.getRootNode().host.callService('media_player', 'select_source', {entity_id: '${ags.entity_id}', source: '${f.name}'})">
        <div class="fav-art-shell"><ha-icon icon="mdi:star" style="color:var(--primary);"></ha-icon></div><div class="fav-label">${this.escapeHtml(f.name)}</div>
      </div>`).join("")}</div></div>`;
  }
  ...
  renderRooms(ags) {
    ...
    return `<div class="rooms-view"><div class="view-title">Groups</div>${r.map(room => `
      <div class="list-card browse-item" style="padding:12px 16px; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center; border-radius:12px;">
        <div><div style="font-weight:800; font-size:1rem;">${this.escapeHtml(room.name)}</div><div style="font-size:0.7rem; color:var(--text-sec);">${room.active?'Active':'Idle'}</div></div>
